...
class TaskUploader:
    ...
    def create_or_update(model, instance=None, params=None):
        ...
        try:
            if instance:
                ...
                if not db_instance:
                    raise ValueError(
                        f"TaskUploader (create_or_update): "
                        f"{model.__tablename__} с ID {instance_id} не найден"
                    )
            ...
            db.rollback()
            raise Exception(
                f"TaskUploader (create_or_update): Ошибка при создании или обновлении: {e}"
            )
        finally:
            db.close()
    ...
    @staticmethod
    def upload_file(content, filename=None, dest='uploaded/'):
        """
        ...
        if not filename:
            try:
                ...
                raise ValueError(
                    "TaskUploader (upload_file): Не указано имя файла для сохранения"
                )
        ...
        if file_ext not in file_factory:
            raise ValueError(
                f"TaskUploader (upload_file): Неизвестное расширение файла: {file_ext}"
            )
    ...
    def process_file(content, dest, filename, parts=True):
        ...
        files = (
            TaskUploader.make_parts(dest, geo_path, filename) if parts else [geo_path]
        )
        TaskUploader.clean_files(dest)
        return files
    ...
    def restart_task(db_task, celery):
        ...
        files_for_delete = [
            db_task.kpt_file,
            db_task.kad_file,
            f'temp/task_{db_task.id}_files.zip',
        ]
