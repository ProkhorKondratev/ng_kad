...
from db import DBTask, DBTasksGroup
from worker import celery, CollectKadTask
...
async def app_lifespan(app: FastAPI):
    ...
    work_dirs = ['database', 'uploaded', 'results', 'temp', 'logs']
    ...
    async with aiosqlite.connect(DATABASE_URL) as db:
        ...
        for row in tasks_rows:
            ...
            if kpt and kpt['state'] == 'FAILED' or kad and kad['state'] == 'FAILED':
                ...
                kpt and kpt['state'] == 'SUCCESS' and kad and kad['state'] == 'SUCCESS'
            ):
                stats['completed'] += 1
            elif (
                kpt
                and kpt['state'] != 'PREPARING'
                or kad
                and kad['state'] != 'PREPARING'
            ):
                stats['in_progress'] += 1
...
async def get_tasks():
    ...
    async with aiosqlite.connect(DATABASE_URL) as db:
        cursor = await db.execute(
            "SELECT id, name, added, kpt_status, kad_status, group_id FROM ngw_tasks"
        )
...
async def download_group_files(group_id: int):
    ...
    archive_name = f"temp/group_{group_id}_files"
    ...
    async with aiosqlite.connect(DATABASE_URL) as db:
        ...
        group_path = f'temp/group_{group_id}'
        await create_archive(group_path, files, archive_name)

        return FileResponse(
            path=archive_name + '.zip', filename=f"{group_files[0][2]}_files.zip"
        )
...
async def download_task_files(task_id: int):
    ...
    """
    os.makedirs('temp', exist_ok=True)
    archive_name = f"temp/task_{task_id}_files"
    ...
    async with aiosqlite.connect(DATABASE_URL) as db:
        cursor = await db.execute(
            "SELECT kpt_file, kad_file, name FROM ngw_tasks WHERE id = ?", (task_id,)
        )
        task_files = await cursor.fetchone()

    task_path = f'temp/task_{task_id}'
    await create_archive(task_path, [file for file in task_files if file], archive_name)

    return FileResponse(
        path=archive_name + '.zip', filename=f"{task_files[2]}_files.zip"
    )
...
async def delete_group(group_id: int):
    async with aiosqlite.connect(DATABASE_URL) as db:
        ...
        files_for_delete.append(f'temp/group_{group_id}_files.zip')
...
async def delete_task(task_id: int):
    async with aiosqlite.connect(DATABASE_URL) as db:
        ...
        if task_files:
            celery.control.revoke(task_files[0], terminate=True)

            files_for_delete = task_files[1:]
            files_for_delete.append(f'temp/task_{task_id}_files.zip')
            await delete_paths(*files_for_delete)

            await execute_db_operations(
                db, ("DELETE FROM ngw_tasks WHERE id = ?", (task_id,))
            )
    ...
    try:
        ...
        for file in files:
            ...
            for path in paths:
                ...
                CollectKadTask().apply_async(
                    args=(db_task.id,), task_id=str(celery_uuid)
                )
...
async def restart_group(group_id: int):
    ...
    async with aiosqlite.connect(DATABASE_URL) as db:
        cursor = await db.execute(
            "SELECT id FROM ngw_tasks WHERE group_id = ?", (group_id,)
        )
        tasks = await cursor.fetchall()
        ...
        TaskUploader.create_or_update(
            model=DBTasksGroup, instance=group_id, params={'added': moscow_time}
        )

        for task in tasks:
            db_task = TaskUploader.restart_task(task[0], celery)
            CollectKadTask().apply_async(
                args=(db_task.id,), task_id=db_task.celery_task
            )
