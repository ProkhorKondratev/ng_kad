...
def check_status(ngw_task_id: str | None, task_type: str, db_task: int | DBTask) -> DBTask:
    task_config = {
        'kpt': {
            'file_key': 'kpt_file',
            'upload_method': TaskUploader.process_file,
            'suffix': '.csv'
        },
        'kad': {
            'file_key': 'kad_file',
            'upload_method': TaskUploader.process_zip,
            'suffix': '.zip'
        },
    }
    ...
    def run(self, *args, **kwargs):
        ...
        try:
            ...
            TaskUploader.create_or_update(
                ...
                instance=db_task,
                params={
                    current_stage: {'state': 'FAILED', 'error': '(Worker): Превышено время выполнения задачи'}
                }
            )
            ...
            TaskUploader.create_or_update(
                ...
                instance=db_task,
                params={
                    current_stage: {'state': 'FAILED', 'error': str(e)}
                }
            )
        return
